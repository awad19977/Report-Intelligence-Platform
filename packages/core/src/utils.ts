import { Logger, Cache, Storage, WorkerClient } from "./types.js";
...
// Type for WorkerClient factory - to be implemented by consumers
export type WorkerClientFactory = () => Promise<WorkerClient>;

// This will be set by the application bootstrap
export let createWorker: WorkerClientFactory | null = null;

export function setWorkerFactory(factory: WorkerClientFactory): void {
  createWorker = factory;
}

export async function getWorker(): Promise<WorkerClient> {
  if (!createWorker) {
    throw new Error("Worker factory not set. Call setWorkerFactory() first.");
  }
  return createWorker();
}